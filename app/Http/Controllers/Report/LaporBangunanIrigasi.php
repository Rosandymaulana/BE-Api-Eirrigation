...
use App\Models\Report\ReportPhotoBuilding;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Validation\ValidationException;

class LaporBangunanIrigasi extends Controller
{
        ...
        try {
            ...
            if (is_array($notes) && is_array($levels)) {
                foreach ($notes as $index => $note) {
                    ...
                    $reportBuilding = ReportBuilding::create([
                        'report_id' => $reportList->id,
                        'building_id' => BangunanIrigasi::inRandomOrder()->first()->id,
                        'level' => $level,
                        'type' => 'RUSAK',
                        'note' => $note,
                        'created_at' => Carbon::now(),
                    ...
                    if (isset($images[$index])) {
                        ...
                        ReportPhotoBuilding::create([
                            'report_building_id' => $reportBuilding->id,
